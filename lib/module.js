...
export default function nuxtRfgIcon(options) {
	...
	if (this.options.dev && process.env.NODE_ENV !== "production") {
		consola.warn(
			"plugin is disabled in dev mode, set NODE_ENV to 'production' to override"
		);
		return;
	}
	...
	this.nuxt.hook("build:before", async builder => {
		const faviconConfig = defaultsDeep(
			this.options["rfg-icon"] || options || {},
			rfgDefaults
		);
		...
		if (faviconConfig.static) {
			...
			if (fs.existsSync(jsonFiles.manifest) && fs.existsSync(jsonFiles.head)) {
				if (faviconConfig.force) {
					...
					return Promise.all(
						...
						.then(() => {
							consola.info(
								"Static files enabled and exists, wont retrieve new favicons"
							);
						})
						.catch(err => {
							/* istanbul ignore next */
			...
			const publicPath =
				this.options.router.base + faviconConfig.staticPath + "/";
		...
		faviconConfig.rfg.masterPicture =
			faviconConfig.masterPicture ||
			path.resolve(this.options.srcDir, "static", "icon.png");
		...
		try {
			...
			const { data: zipData } = await rfgApiRequestPackage(
				faviconConfig,
				metaData.favicon_generation_result.favicon.package_url
			);
			if (!zipData) {
			...
			fs.writeFileSync(
				path.resolve(this.options.srcDir, "static", "favicon.ico"),
				favicon.buff,
				err => {
					/* istanbul ignore next */
					if (err) {
			...
			if (faviconConfig.static) {
				...
				faviconFiles.push({
					...
					buff: JSON.stringify(options.head)
				});
				...
				return Promise.all(
					faviconFiles.map(
						file =>
							new Promise((resolve, reject) => {
								fs.writeFile(
									path.resolve(iconDir, file.fileName),
									file.buff,
									err => {
										/* istanbul ignore if */
										if (err) {
