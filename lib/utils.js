...
const init = rfgInit.init
	.toString()
	.replace(/require\(['"]([^)]+)['"]\)/g, (string, module) => {
...
export const rfgApiRequestPackage = (faviconConfig, packageUrl) => {
	if (faviconConfig.rfgApiPackageResponse) {
		return Promise.resolve({
			data: convertToBuffer(faviconConfig.rfgApiPackageResponse)
		});
	}
			...
			if (entries.length) {
				...
				const wait = data => {
					return new Promise(resolve => {
						setTimeout(() => {
							if (
								entries.length !== faviconFiles.length ||
								entries.length < zipfile.entryCount
							) {
								p = p.then(() => {
									wait(data);
								});
